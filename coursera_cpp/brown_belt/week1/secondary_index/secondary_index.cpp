...
class Database {
 public:
  bool Put(const Record& record) {
    ...
    auto indexTimeStampIt =
        indexTimeStamp_.emplace(record.timestamp, storageIt);
    auto indexKarmaIt = indexKarma_.emplace(record.karma, storageIt);
    indexID_.emplace(record.id, DataIterators{storageIt, indexUserIt,
                                              indexTimeStampIt, indexKarmaIt});
    return true;
  }
