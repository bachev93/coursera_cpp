...
struct AddressHasher {
    ...
    size_t operator()(const Address& address) const {
        //Ax^2 + Bx + C
        const auto a = shash_(address.city);
        const auto b = shash_(address.street);
        const auto c = ihash_(address.building);
...
struct PersonHasher {
    ...
    size_t operator()(const Person& person) const {
        //Ax^3 + Bx^2 + C*x + D
        const auto a = shash_(person.name);
        const auto b = ihash_(person.height);
        const auto c = dhash_(person.weight);
        const auto d = addressHash_(person.address);
