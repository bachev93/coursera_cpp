...
class Booker {
 public:
  explicit Booker()
      : reservations_(), reservedRooms_(), uniqueHotelClients_() {}
  ...
  void book(const Reservation& res) {
    ...
    auto timeDiff =
        abs(reservations_.back().time_ - reservations_.front().time_);
 ...
 private:
  ...
  void adjust(const Reservation& oldRes) {
    ...
    auto& clientReservations =
        uniqueHotelClients_[oldRes.hotelName_][oldRes.clientID_];
    --clientReservations;
