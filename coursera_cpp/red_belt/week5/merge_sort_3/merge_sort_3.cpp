...
void MergeSort(RandomIt range_begin, RandomIt range_end) {
  ...
  merge(make_move_iterator(res.begin()), make_move_iterator(oneThird),
        make_move_iterator(oneThird), make_move_iterator(twoThird),
        back_inserter(tmp));

  // 6. С помощью алгоритма merge сливаем отсортированные части в исходный
  // диапазон
  merge(make_move_iterator(tmp.begin()), make_move_iterator(tmp.end()),
        make_move_iterator(twoThird), make_move_iterator(res.end()),
        range_begin);
}
