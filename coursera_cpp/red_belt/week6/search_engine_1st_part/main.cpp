...
void testProductivity() {
  const vector<string> initialDocs = {
      "london is the capital of great britain",
      "paris is the capital of france",
      "berlin is the capital of germany",
      "rome is the capital of italy",
  ...
  for (auto i = 0, I = 1000; i < I; ++i) {
    docs.insert(end(docs), begin(initialDocs), end(initialDocs));
  }

  vector<string> queries(initialQueries);

  for (auto i = 0, I = 10; i < I; ++i) {
    queries.insert(end(queries), begin(initialQueries), end(initialQueries));
  }

  istringstream docs_input(Join('\n', docs));
  istringstream queries_input(Join('n', queries));
...
void testError() {
  const vector<string> docs = {"a   b c   d", "  a b c d", "   c  d e f   ",
                               "d  e f    g"};

  const vector<string> queries = {" c d"};

  const vector<string> expected = {Join(
      ' ', vector{"c d:", "{docid: 0, hitcount: 2}", "{docid: 1, hitcount: 2}",
                  "{docid: 2, hitcount: 2}", "{docid: 3, hitcount: 1}"})};
