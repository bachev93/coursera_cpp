...
class AirportCounter {
 public:
  ...
  AirportCounter() : statistics_() {}
  //    AirportCounter() = default;

  // конструктор от диапазона элементов типа TAirport
  template <typename TIterator>
  AirportCounter(TIterator begin, TIterator end) : statistics_() {
    for (auto it = begin; it != end; ++it) {
      ++statistics_[static_cast<size_t>(*it)];
    }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
    return statistics_[static_cast<size_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) { ++statistics_[static_cast<size_t>(airport)]; }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport) {
    if (statistics_[static_cast<size_t>(airport)] > 0) {
      --statistics_[static_cast<size_t>(airport)];
    } else {
      EraseAll(airport);
    }
  }
  ...
  void EraseAll(TAirport airport) {
    statistics_[static_cast<size_t>(airport)] = 0;
  }
  ...
  Items GetItems() const {
    Items res;
    for (size_t i = 0, I = res.size(); i < I; ++i) {
      res[i] = make_pair(static_cast<TAirport>(i), statistics_[i]);
    }
 ...
 private:
  array<size_t, static_cast<size_t>(TAirport::Last_)> statistics_;
};
